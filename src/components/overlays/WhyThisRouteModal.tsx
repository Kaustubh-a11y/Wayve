...
import { ArrowDownRight, ArrowUpRight, BarChart3, HelpCircle, Sparkles, X } from "lucide-react";
...
export const WhyThisRouteModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-lg glass-panel rounded-t-3xl sm:rounded-3xl p-5 sm:p-6 shadow-2xl border border-white/10 z-10 flex flex-col gap-4 max-h-[88vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-200/80 dark:border-slate-800/80">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white">Why Wayve Chose This Route</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">Explainable AI (XAI) & optimization transparency</p>
            </div>
          </div>
          <button
            onClick={() => toggleWhyThisRoute(false)}
        ...
        {/* Highlight Summary */}
        <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-850/80 border border-slate-200 dark:border-slate-800 flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">
              Selected Route
            </span>
            <span className="text-xs font-bold px-3 py-0.5 rounded-full bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 font-mono">
              Confidence {active.confidence}%
            </span>
          </div>
          <h3 className="text-base font-bold text-slate-900 dark:text-white">{active.name}</h3>
        ...
        <div>
          <span className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider flex items-center gap-1.5 mb-2.5">
          ...
          <div className="flex flex-col gap-2">
            {attributions.map((attr, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-850/60 border border-slate-200 dark:border-slate-800/80 text-xs"
              >
                <div className="flex items-center gap-2">
                  {attr.direction === "decrease" ? (
                    ...
                    <div className="w-5 h-5 rounded-full bg-amber-500/15 text-amber-600 dark:text-amber-400 flex items-center justify-center">
                      <ArrowUpRight className="w-3.5 h-3.5" />
                    </div>
                  )}
                  <span className="font-medium text-slate-700 dark:text-slate-200">{attr.feature}</span>
                </div>

                <span
                  className={`font-mono font-bold ${
                    attr.direction === "decrease" ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"
                  }`}
                >
                  {attr.direction === "decrease" ? "-" : "+"}
        ...
        <div className="pt-2 border-t border-slate-200/80 dark:border-slate-800/80">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {[
              ...
              <div key={idx} className="p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-850/70 border border-slate-200 dark:border-slate-800">
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-slate-500 dark:text-slate-400 text-[11px] font-semibold">{factor.label}</span>
                  <span className="text-emerald-600 dark:text-emerald-400 font-mono font-bold">{factor.score}</span>
                </div>
